...
## 2025年12月26号重要更新
- 突然发现可能是账号在裸奔，因为github action的执行结果是公开的，包括截图，之前没登录账号看不了以为不是公开的，所以放心传截图上去，这样安全性不好，
- 各位请及时拉取最新的分支，然后运行delete-workflow-runs.yml删除所有执行记录，或者转为私库，之后使用github action的记录不会再上传截图。
## 使用方法
克隆本项目到本地
```bash
#克隆项目
...
- 3、遗留问题：
登录新面板第一次会有跳过二次认证选项，那个按钮的xpath是在英文环境下浏览器情况下完成的，如果第一次遇到中文环境可能无法跳过。

其现在如下
```
skip = inpage.ele('x://div[@role="button" and normalize-space(.)="Skip"]', timeout=30)
```
因为我的账号已经不跳这个选项了，不好定位修复这个问题，有开发能力的人可以pull request。

**使用docker或者运行环境为英文的浏览器不受此问题影响。**
